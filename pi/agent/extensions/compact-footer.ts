    ...
    ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          const thinkColor = thinking === "off" ? "muted" : thinking === "minimal" ? "dim" : thinking === "low" ? "dim" : thinking === "medium" ? "accent" : thinking === "high" ? "warning" : "error";
          const modelId = ctx.model?.id || "no-model";
          const thinkingText = thinking && thinking !== "off" ? theme.fg(thinkColor, `thinking:${thinking}`) : theme.fg("muted", "thinking:off");
          const modelStr = theme.fg("accent", modelId) + " " + thinkingText;
